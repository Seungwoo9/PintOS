...
{
  char *fn_copy;
  char token[256];      //PJT2-3
  struct list_elem* e;  //PJT2-4
  struct thread* t;     //PJT2-4
  tid_t tid; 
  ...
  parse_filename(file_name, token); //PJT2-3
  ...
  tid = thread_create (token, PRI_DEFAULT, start_process, fn_copy); //PJT2-3
  struct thread * c = get_child(tid);
  if( c == NULL) {
        return -1;
  }
  sema_down(&thread_current()->load_lock); //PJT2-4
  if (tid == TID_ERROR)
  ...
  for (e = list_begin(&thread_current()->child); e != list_end(&thread_current()->child); e = list_next(e)) {
     ...
     if (t->tid == tid && t->successfully_load == false) {
        list_remove(&t->child_elem);
        //return process_wait(tid);
      }
   }
  */
  if(c->successfully_load == 0){
        remove_child(c);
        return -1;
  }  
  //PJT2-4
...
start_process (void *file_name_)
{
  ...
  success = load (token, &if_.eip, &if_.esp); //PJT2-3
  
  //PJT2-3
  if (success) {
     construct_stack (file_name, &if_.esp);
     t->successfully_load = true;
  }
  else
    t->successfully_load = false;
  //PJT2-3
  
  /* If load failed, quit. */
  palloc_free_page (file_name);
  sema_up(&thread_current()->parent->load_lock); //PJT2-4
  if (!success) {    
     thread_exit ();
  }
  /* Start the user process by simulating a return from an
     ...
     and jump to it. */
  //hex_dump(if_.esp, if_.esp, PHYS_BASE - if_.esp, true); //PJT2-3
...
process_exit (void)
{
  ...
  sema_up(&(cur->child_lock)); //PJT2-4
  sema_down(&(cur->mem_lock)); //PJT2-4
}
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  char *save_ptr;
  int argc = 0;
  char *argv[256]; // arbitrary limit on the number of arguments
  char *arg;
...
void parse_filename(char *src, char *dummy) {
  int i;
  strlcpy(dummy, src, strlen(src) + 1);
  for (i=0; dummy[i]!='\0' && dummy[i] != ' '; i++);
  dummy[i] = '\0';
}
